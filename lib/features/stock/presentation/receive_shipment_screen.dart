...
class _ReceiveShipmentScreenState extends ConsumerState<ReceiveShipmentScreen> {
  ...
  void _initialize(Shipment shipment) {
    for (final item in shipment.items) {
      for (final copy in item.copies.where((c) => c.status == 'DISTRIBUTED')) {
        _received.putIfAbsent(copy.id, () => true);
        _discrepancies.putIfAbsent(copy.id, () => ReceiptDiscrepancy.none);
      }
    }
  }
                ...
                children: [
                  ...
                  FilledButton(
                    onPressed: _submitting
                        ? null
                        : _step == 2
                        ? (receivedCount == 0 ? null : () => _confirm(shipment))
                  ...
                  children: [
                    ...
                    for (final item in shipment.items)
                      ListTile(
                        ...
                        title: Text(item.edition.title),
                        subtitle: Text(
                          '${item.edition.format} · ${item.edition.isbn}',
                        ),
                        trailing: Text('${item.quantity} expected'),
                      ),
                  ],
                ),
              ),
                          ...
                          child: Column(
                            children: [
                              CheckboxListTile(
                                ...
                                subtitle: Text(row.item.edition.isbn),
                                onChanged: (value) => setState(
                                  () => _received[row.copy.id] = value ?? false,
                                ),
                              ),
                              DropdownButtonFormField<ReceiptDiscrepancy>(
                                initialValue: _discrepancies[row.copy.id],
